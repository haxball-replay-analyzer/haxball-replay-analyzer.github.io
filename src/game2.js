...
var toucher = { name: "", team: "0" }, lastToucher = "", mtc = 0, playSounds = false, pileczka = [], keepUpdating = true, keepUpdatingStadium = true;
...
function playersToFollowList() {
  ...
  var b = '<option value=0>[0] ball</option>';
  for (var i = 0; i < players.length; i++) {
    if (players[i].V == 0) continue;
    a.push({
      id: players[i].V,
      name: players[i].w
    });
    b = b + '<option value=' + a[a.length - 1].id + (playerToFollow == a[a.length - 1].id ? ' selected=true> [' : ' > [') + a[a.length - 1].id + '] ' + a[a.length - 1].name + '</option > ';
  }
  return b;
}
...
function sortTable(par) {
  ...
  */
  ...
  document.getElementById('prevMatch').style = 'position:relative;top:-25px';
  document.getElementById('nextMatch').style = 'position:relative;top:-25px';
  if (mtc == match.length - 1) document.getElementById("nextMatch").style = "background-color: grey;position:relative;top:-25px";
  if (mtc == 0) document.getElementById("prevMatch").style = "background-color: grey;position:relative;top:-25px";
  keepUpdating = false;
...
Ba.b = !0;
Ba.cg = function (a) {
  // CHECKPOINT overwrite bo nie mogę perfectscrollbara
  return true;
};
mc.b = !0;
mc.ts = function () {
...
Gb.prototype = {
  ...
  ri: function (a) {
    ...
    a.Oi = function (a) {
      ...
      if (keepUpdating) match[match.length - 1].stopped = true;
    ...
    a.xl = function (d, e, f) {
      checkTeams = true;
      if (keepUpdating && match[match.length - 1].started && !match[match.length - 1].stopped && match[match.length - 1].gameTicks > 0) {
        match[match.length - 1].changes.push([e.w, f.w, parseTime(match[match.length - 1].gameTicks)]);
      }
      null != a.K && c.j.Qa.Gb('' +
...
da.prototype = C(m.prototype, {
  ...
  Kc: function (a, b) {
    ...
    if (null != a.K) {
      ...
      // console.log(a.I[1].H.D, a.I[1].H.a)
      // a.I[i].V - id gracza
      // a.I[i].H nie może być null
